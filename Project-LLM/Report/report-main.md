...
<!-- Main body (Part 6 mini-project). Drafted: Abstract, Introduction, Problem description. To come: Techniques & methods, Results & discussion, Conclusion. Figures live in ../../mini-project-DoRA/figures/. -->

## Abstract

Parameter-efficient fine-tuning (PEFT) adapts large language models to new tasks by training a tiny fraction of their weights. Low-Rank Adaptation (LoRA) is the standard choice, but Weight-Decomposed Low-Rank Adaptation (DoRA; Liu et al., ICML 2024) reports consistently higher accuracy — with its largest gains at low rank — by splitting each weight matrix into a magnitude and a direction and applying the low-rank update only to the direction. We reproduce DoRA's central claim as a *regime-contrast generalization study*: instead of the paper's raw LLaMA base, we adapt the instruction-tuned `Mistral-7B-Instruct-v0.3` on the per-task BoolQ benchmark and the multi-task commonsense-170k mixture, sweeping rank $r \in \{4, 8, 16\}$ against a matched LoRA baseline, at a course-feasible 2,500-step budget (7.8% of the paper's, three seeds) and a 10,000-step extension (31%, four fresh seeds). Across 60 training runs on University of Melbourne Spartan H100 GPUs, we find that DoRA does **not** improve task accuracy over LoRA at any rank — no rank shows a DoRA advantage beyond seed noise — but at mid rank ($r{=}8$) it **better resists the format shift** that multi-task training induces — retaining the original yes/no answer format where LoRA adopts the trained true/false (likelihood $+0.16$, outside seed std, three of four seeds agreeing). Quadrupling the training budget sharpens an *inverse-rank* generalization trend — low-rank adapters stay at or above the zero-shot baseline (genmatch $0.82$) while high-rank adapters drift well below it (the within-method gap widening from $\sim 3$–$7$ to $\sim 14$–$16$ pp). A parser fix that disentangles *task accuracy* from *format adaptation* proved essential to reading these results correctly, and DoRA's $\sim 3.3\times$ wall-time and $\sim 1.8\times$ peak-memory overhead is reconfirmed throughout. We conclude that DoRA's low-rank advantage surfaces here on a format-related axis rather than as accuracy, and we discuss how the limited seed count, the sub-paper training budget, and the already-aligned instruction-tuned base each temper that conclusion.
...
**Model and data.** The base model is `Mistral-7B-Instruct-v0.3` (7.25 B parameters, instruction-tuned, open-access and already part of the course toolchain). We adapt it in two regimes:
...
- **commonsense-170k** (cs170k) — a 170,000-example mixture of eight commonsense-reasoning tasks (BoolQ among them), assembled by the LLM-Adapters project and used by the DoRA paper. This is the *multi-task* regime, and the one the paper's low-rank claim is built on.
...
$$
freezing $W_0$ and training only the rank-$r$ factors $A$ and $B$; the scalar $\alpha/r$ keeps the update's scale stable as $r$ varies. **DoRA** keeps this low-rank update but first splits each weight into a *magnitude* and a *direction*. Writing $\lVert \cdot \rVert_c$ for the column-wise Euclidean norm, it reparameterizes the adapted weight as
$$
...
For a fair comparison the two methods share an otherwise identical configuration: we use HuggingFace `peft`'s `LoraConfig`, and the *only* difference between a LoRA run and its DoRA counterpart is the `use_dora` flag. We deliberately use the library implementation rather than the authors' bundled fork — the question is whether a practitioner reaching for the standard tool sees the paper's advantage.
...
Quadrupling the cs170k budget to 10k steps (four fresh seeds) sharpens both findings. The inverse-rank trend widens from $\sim 3$–$7$ pp (3.3 pp LoRA, 6.6 pp DoRA) to $\sim 14$ pp (LoRA) / $\sim 16$ pp (DoRA) within-method (Figure 4): r=4 still sits at or above baseline ($0.84$/$0.83$) while r=16 drops to $0.70$/$0.67$. More training simply lets higher-rank adapters commit harder to the multi-task distribution and sacrifice more BoolQ-specific accuracy — a roughly $2$–$4\times$ amplification of the Tier-2 signal ($\approx 4\times$ for LoRA, $\approx 2.5\times$ for DoRA).
