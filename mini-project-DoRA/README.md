...
2. **DoRA shows a small cell-mean edge on format-adaptation strength** — the *strict-parser* cs170k genmatch — of +0.056 at $r \in \{8, 16\}$. Seed std is large (0.16–0.35) and per-seed direction is split (DoRA wins 2 of 3 seeds at $r{=}8$, only 1 of 3 at $r{=}16$, where the cell mean is driven by seed-2 alone). A hint matching the paper's central hypothesis on format adaptation, well below significance at $n{=}3$.
...
- **$n{=}3$ seeds is low.** Especially for the strict-parser metric where seed std is 0.16–0.35, the +0.056 DoRA cell-mean edge at $r \in \{8, 16\}$ sits within seed std and per-seed direction is split (DoRA wins 2/3 at $r{=}8$, only 1/3 at $r{=}16$) — a hint matching the paper's hypothesis, well below formal significance. More seeds would clarify. *Tier 3 partially addresses this with a separate n=4 enrichment at 10k steps; the refined picture is in [results/SUMMARY-tier3.md](results/SUMMARY-tier3.md).*
