...
**Format-adaptation strength (strict genmatch).** Here the DoRA − LoRA cell-mean is +0.056 at r=8 and r=16, with both r=4 cells tied near the floor (~0.10). The seed std on this metric is large (~0.16–0.35 per cell, comparable to the means) and the per-seed direction is split: at r=8 DoRA wins 2 of 3 seeds, at r=16 only 1 of 3 (the r=16 cell-mean is driven by seed-2 alone, where DoRA leads by +0.39 while LoRA leads in the other two). Well below statistical significance at n=3 — a hint matching the spec's central hypothesis on format adaptation (DoRA's magnitude/direction decomposition embeds the cs170k true/false training format more readily than LoRA), not significant evidence. The effect does not translate into measurable task-accuracy improvement at our 2500-step training budget.
